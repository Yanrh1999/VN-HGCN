...
class HAN(BaseModel):
    ...
    def __init__(self, ntype_meta_paths_dict, in_dim, hidden_dim, out_dim, num_heads, dropout, drop_edge):
        super(HAN, self).__init__()
        self.out_dim = out_dim
        self.mod_dict = nn.ModuleDict()
        for ntype, meta_paths_dict in ntype_meta_paths_dict.items():
            self.mod_dict[ntype] = _HAN(meta_paths_dict, in_dim, hidden_dim, out_dim, num_heads, dropout, drop_edge)
...
class _HAN(nn.Module):
    ...
    def __init__(self, meta_paths_dict, in_dim, hidden_dim, out_dim, num_heads, dropout, drop_edge):
        super(_HAN, self).__init__()
        self.layers = nn.ModuleList()
        self.layers.append(HANLayer(meta_paths_dict, in_dim, hidden_dim, num_heads[0], dropout, drop_edge))
        for l in range(1, len(num_heads)):
            self.layers.append(HANLayer(meta_paths_dict, hidden_dim * num_heads[l - 1],
                                        hidden_dim, num_heads[l], dropout, drop_edge))
...
class HANLayer(nn.Module):
    ...
    def __init__(self, meta_paths_dict, in_dim, out_dim, layer_num_heads, dropout, drop_edge):
